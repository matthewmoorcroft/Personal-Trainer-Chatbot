...
class Database:
    ...
    def __init__(self):
        try:
            self.conn = psycopg2.connect("dbname='pt' user='matthew' host='localhost' password='1245788956'")
        except:
    ...
    def check_user(self, telegram_id):
        ...
        if user = None:
            ...
            if rowcount:
                ...
                user = User(id, name, age, gender, telegram_id, weight, bodyfatratio,
                        c_chest, c_leg, c_waist, c_triceps)
    ...
    def add_log(self, id_to = 'Sam', id_from = 'Sam', message, intent, score = -1):
        db.update_processed(row_id)
        cur.execute("INSERT INTO core.interaction_log (id_to, id_from, message, intent, score) VALUES ('{id_to}','{id_from}','{message}','{intent}',{score})")
        conn.commit()
        cur.close()
